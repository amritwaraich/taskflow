...
import { Plus, Search, CheckCircle, Circle, Clock, Tag, MoreHorizontal, Trash2 } from 'lucide-react'
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      ...
      const { data: tasks } = await supabase
        .from('tasks')
        .select('*, labels:task_labels(label:labels(*))')
        .eq('user_id', user.id)
        .is('project_id', null)
        .order('position')
  ...
  return (
    <div className="max-w-3xl mx-auto p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <span className="text-sm text-gray-500">{tasks.filter(t => !t.is_completed).length} tasks</span>
      </div>
          ...
          {tasks.map((task) => (
            ...
            >
              <button
                ...
                className={`flex-shrink-0 ${task.is_completed ? 'text-violet-600' : 'text-gray-400 hover:text-violet-600'}`}
              >
                {task.is_completed ? (
                ...
                <div className="flex items-center gap-3 mt-1">
                  {task.due_date && (
                    <span className={`text-xs flex items-center gap-1 ${
                      isPast(new Date(task.due_date)) && !task.is_completed ? 'text-red-500' : 'text-gray-500'
                    }`}>
